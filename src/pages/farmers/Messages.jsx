...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    const myThreads = threads.filter((t) => t.farmerId === farmerId);
    const mapped = myThreads.map((t) => ({
      id: t.id,
      contactName: t.ownerName,
      equipment: t.equipmentName,
      unread: t.unreadForFarmer || 0,
      messages: t.messages || [],
    }));
    setContacts(mapped);
  ...
  const sendMessage = () => {
    ...
    threads[idx].unreadForOwner = (threads[idx].unreadForOwner || 0) + 1;
    setStored(STORAGE_KEYS.chats, threads);
    setContacts((prev) =>
      prev.map((c) =>
        c.id === activeThread.id ? { ...c, messages: [...c.messages, newMsg] } : c
      )
    );
    setInput("");
    setTimeout(() => {
